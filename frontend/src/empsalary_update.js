...
const nav=useNavigate();
    const salaryupdate=(e)=>{
     ...
     const details={
      ...
      axios.post(`http://localhost:6600/employeesalaryupdate/${id}`,details).then((res)=>{
          if(res!==''){
...
<div style={{height:'350px',width:'450px',background:'whitesmoke',borderRadius:'20px',marginTop:'200px',marginLeft:'35%',boxShadow:'2px 2px 2px grey'}}>
  ...
  <div  style={{background:'whitesmoke',marginLeft:'20px',marginTop:'30px'}}>
                   <label style={{fontSize:'20px',paddingLeft:'10px',fontWeight:'bold'}}> <FaRegMoneyBillAlt />Salary</label><br/><input style={{height:'50px',borderRadius:'5px',background:'whitesmoke',width:'90%',marginTop:'10px',border:'none',boxShadow:'2px 2px 2px grey'}} type='number'  value={salary} name='salary'  placeholder='Previous salary'  onChange={handler} />
</div>
